"""Core Agent class for the Python AI Agent."""

import logging
from typing import Any, Callable, Protocol

import anthropic
from anthropic.types import Message, MessageParam, ToolResultBlockParam, ToolParam
...
class Agent:
    ...
    def __init__(
        ...
        verbose: bool = False,
        get_user_input: Callable[[], str] | None = None,
    ):
        """
    ...
    def run(self) -> None:
        ...
        if self.verbose:
            self.logger.info(
                f"Starting chat session with {len(self.tools)} tools enabled"
            )
    ...
    def _process_conversation(self) -> None:
        ...
        if self.verbose:
            self.logger.info(
                f"Sending message to Claude, conversation length: {len(self.conversation)}"
            )

        # Make API call to Claude
        try:
            message = self._call_claude()
            # Convert Message to MessageParam for conversation history
            message_param = MessageParam(role=message.role, content=message.content)
            self.conversation.append(message_param)
        ...
        while True:
            ...
            if self.verbose:
                self.logger.info(
                    f"Processing {len(message.content)} content blocks from Claude"
                )
            ...
            if tool_results:
                tool_message = MessageParam(role="user", content=tool_results)
    ...
    def _call_claude(self) -> Message:
        ...
        response = self.client.messages.create(
            ...
            if anthropic_tools
            else NOT_GIVEN,  # Use NOT_GIVEN to avoid sending empty tools
        )
